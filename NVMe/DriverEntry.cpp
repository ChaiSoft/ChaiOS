...
public:
	...
	void init()
	{
		...
		m_adminCommandQueue = new CommandQueue(this, pasq, admin_queue_size, NVME_ADMIN_QUEUE_DOORBELL);
		if (!queue_valid(m_adminCommandQueue))
			return kprintf(u"Error allocating NVMe ASQ\n");

		m_adminCompletionQueue = new CompletionQueue(this, pacq, admin_queue_size, NVME_ADMIN_QUEUE_DOORBELL);
		if (!queue_valid(m_adminCommandQueue))
		...
		PNVME_COMPLETION completion = m_adminCompletionQueue->wait_event(1000, token);
		...
		for (unsigned i = 1; i <= m_countIoCompletion; ++i)
		{
			...
			token = m_adminCommandQueue->submit_entry(cmd);
			PNVME_COMPLETION createio = m_adminCompletionQueue->wait_event(1000, token);
			if (i == 1)
			{
...
#pragma pack(pop)
	...
	static const uint32_t NVME_ADMIN_QUEUE_DOORBELL = 0;
	...
	public:
		...
		PNVME_COMPLETION wait_event(uint64_t timeout, uint16_t cmdid)
		{
			++m_waiting;
			...
			m_list_head = next_after(m_list_head);
			--m_waiting;
