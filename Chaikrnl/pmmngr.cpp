...
static uint64_t max_phy_addr = 0;
...
typedef RedBlackTree<uint64_t, MemoryRegionInfo*> numa_memory_info;
...
typedef void(*memory_map_callback)(uint64_t start, uint64_t length, bool usable, bool free, void* param);
...
static MemoryRegionInfo* get_memory_region(uint64_t base)
{
	/*
	if (meminf.empty())
		return &default_info;
...
static void split_on_boundary(uint64_t bound)
{
...
static cache_colour GetCacheColour(uint64_t paddr)
{
	return (paddr / PAGESIZE) % num_colours;
}

static page* GetPFD(uint64_t paddr)
{
	return &PFD[paddr / PAGESIZE];
}
static uint64_t GetPaddr(page* pg)
{
...
static void NumaPopulatePmmInfo(uint64_t start, uint64_t length, bool usable, bool free, void* param)
{
	typedef RedBlackTree<numa_t, uint64_t> map_type;
...
#if defined X86 || defined X64
static uint8_t GetMemRegion(uint64_t addr)
{
	if (addr < 16 * MB)
...
static void FillPageDatabase(uint64_t start, uint64_t length, bool usable, bool free, void* param)
{
	uint64_t pages_in_domain = 0;
	while (length > 0) {
		MemoryRegionInfo* rinfo = get_memory_region(start);
		...
		if (free)
		{
			...
			{
				uint64_t cur_addr = start + n * PAGESIZE;
				cache_colour colour = GetCacheColour(cur_addr);
				uint64_t colour_end = start + length;
				...
				numa_alloc[colour].join_existing_list(GetPFD(cur_addr), GetPFD(colour_end));
			}
		}
		else
		{
...
#ifdef X64
static void* PFD_SLOT = (void*)0xFFFFD00000000000;
#else
...
static page* create_pfd(size_t entries)
{
	...
	while (!check_free(PFD_SLOT, act_sz))
	{
		PFD_SLOT = raw_offset<void*>(PFD_SLOT, act_sz);
	}
	paging_map(PFD_SLOT, PADDR_ALLOCATE, act_sz, PAGE_ATTRIBUTE_WRITABLE | PAGE_ATTRIBUTE_NO_EXECUTE);
...
static void uefi_startup(void* memmap)
{
	...
	RedBlackTree<numa_t, uint64_t> numa_domain_sizes;
	...
	for (uint_fast8_t region = 0; region < ARCH_PHY_REGIONS_MAX; ++region)
	{
		...
		for (RedBlackTree<numa_t, uint64_t>::iterator it = numa_domain_sizes.begin(); it != numa_domain_sizes.end(); ++it)
		{
